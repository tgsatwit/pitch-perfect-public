...
interface GraphData {
  ...
  streamMessage: (params: GraphInput, targetThreadId?: string) => Promise<void>;
...
export function GraphProvider({ children }: { children: ReactNode }) {
  ...
  const updateArtifact = async (
    ...
    // Always update local state first to ensure responsive UI
    ...
    try {
      ...
      // Successfully saved to LangGraph server
      console.log("Artifact successfully saved to LangGraph server");
    } catch (stateError) {
      // Gracefully handle LangGraph connectivity issues
      const errorMessage = stateError instanceof Error ? stateError.message : String(stateError);
      console.log("LangGraph server unavailable - using local-only mode:", errorMessage);
      
      // Show a subtle notification for developers (not users) to understand the state
      if (process.env.NODE_ENV === 'development') {
        console.info("Operating in local-only mode - Canvas changes are saved locally but not synced to LangGraph server");
      }
      
      // The artifact is already marked as saved above, so the UI will show "Saved"
      // This maintains a smooth user experience even when LangGraph is unavailable
    }
  };
  ...
  const streamMessageV2 = async (params: GraphInput, targetThreadId?: string) => {
    ...
    let currentThreadId = targetThreadId ?? threadData.threadId;
    ...
    if (!currentThreadId) {
       toast({ title: "Error", description: "No valid thread ID found", variant: "destructive", duration: 5000 });
       setIsStreaming(false);
       return;
    }

    // Extract configurable from params if present (added as a special property by components)
    const { configurable: inputConfigurable, ...standardParams } = params as any;

    const messagesInput = {
      messages: standardParams.messages,
      _messages: standardParams.messages,
    };
    ...
    try {
      ...
      const stream = workerService.streamData({
        ...
        configurable: {
          ...
          supabase_user_id: userData.user?.id || 'anonymous',
          // Include any other configurable properties passed by the components
          ...inputConfigurable
        }
      });
