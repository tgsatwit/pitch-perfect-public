...
// Define a flexible user type that works with both Firebase and Supabase users
export interface UserType {
  id: string;
  // Add any other properties that are used in this component
}
...
export function CustomQuickActions(props: CustomQuickActionsProps) {
  const { user, assistantId, streamMessage, userLoading } = props;
  const {
  ...
  const handleDelete = async (id: string) => {
    if (!user) {
      toast({
        ...
        description: "Please ensure you're logged in to delete custom quick actions.",
        variant: "destructive",
        duration: 5000,
      });
      return;
        ...
        <TooltipIconButton
          tooltip={
            props.isTextSelected
              ...
              : !user
              ? "Please log in to use custom quick actions"
