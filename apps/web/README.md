...
2. Enable the authentication methods you want to use (Email/Password, Google, GitHub, etc.)
3. Add your web app to the Firebase project to get the configuration values
4. Set up the Firebase Admin SDK service account credentials
5. Update your environment variables with the Firebase configuration
...
The application includes a migration script to move users from Supabase to Firebase. To run the migration:

1. Ensure both Supabase and Firebase environment variables are set in your `.env.local` file
...
3. Run the migration script:
   ```
   npx ts-node --project tsconfig.json scripts/migrateUsers.ts
   ```

The script will:
- Fetch all users from Supabase
- Create corresponding accounts in Firebase
- Generate password reset links for users to set new passwords
- Log the password reset links (in a production environment, you would email these to users)
